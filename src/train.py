...
from .nari_model import Nari, NariConfig, shard_nari, count_params
from .nari_data import DataLoaderConfig
...
def get_batch(
    ...
    y = np.take(data, np.arange(1, block_size + 1) + ix[:, None], axis=0).astype(
        np.int32
    )
...
def make_training_fns(
    ...
    def step(model: Nari, opt_state, x_GxBxT: Array, y_GxBxT: Array, key: KeyArray):
        ...
        def microstep(grad_so_far, xykey_g: tp.Tuple[Array, Array, KeyArray]):
            ...
            grad = shard_nari(grad, mesh, config.shard_model)
    ...
    @eqx.filter_jit
    def simple_loss(
        model: tp.Union[Nari, eqx.Partial],
        x: Array,
        y: Array,
        key: tp.Optional[KeyArray],
    ) -> Array:
    ...
    def evaluate(model: Nari, data: np.ndarray) -> float:
        eval_model = eqx.Partial(
            cast_pytree(model, jnp.dtype(config.compute_dtype)), inference=True
        )
        tot_loss = 0
        num_eval_steps = 1 if config.debug else 200
        for i in range(num_eval_steps):
            x_1xBxD_np, y_1xBxD_np = get_batch(
                data, config.model_config.block_size, config.batch_size, 1
            )
...
def train(config: ExperimentConfig):
    ...
    mesh = Mesh(
        mesh_utils.create_device_mesh((n_devices // 8, 8)),
        axis_names=("replica", "data"),
    )

    train_data = np.memmap(
        os.path.join(config.data_dir, "train.bin"), dtype=np.uint16, mode="r"
    ).copy()
    val_data = np.memmap(
        os.path.join(config.data_dir, "val.bin"), dtype=np.uint16, mode="r"
    ).copy()
    ...
    print(
        f"Process {proc_idx}/{n_proc}. train_data.shape={train_data.shape}, val_data.shape={val_data.shape}."
    )
    ...
    def init_model(model_key):
        model = Nari(config.model_config, model_key)
        model = cast_pytree(model, config.param_dtype)
        model = shard_nari(model, mesh, config.shard_model)
        return model
    ...
    first_step = 0
    if (
        not config.debug and mngr.latest_step() is not None
    ):  # Restore existing checkpoint.
        ex_state = (jtu.tree_leaves(model), jtu.tree_leaves(opt_state))
        ex_shardings = jtu.tree_map(
            lambda x: x.sharding if eqx.is_array(x) else None, ex_state
        )
        restore_args = ocp.checkpoint_utils.construct_restore_args(
            ex_state, ex_shardings
        )
    ...
    for itr in pbar:
        ...
        if pbar.format_dict["rate"] is not None:
            postfix_values["thpt"] = (
                pbar.format_dict["rate"] * config.batch_size * config.g_accum_iters
            )
