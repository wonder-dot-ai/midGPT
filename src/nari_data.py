...
def load_pair(
    text_file: tf.Tensor, audio_file: tf.Tensor
...
def process_sample(
    text: tf.Tensor, audio: tf.Tensor, config: DataLoaderConfig
) -> tp.Tuple[tf.Tensor, tf.Tensor]:
    """
...
def build_delay_indices(
    batch_size: int, audio_length: int, delay_pattern: tp.List[int]
) -> tp.Tuple[tf.Tensor, tf.Tensor]:
    """
    ...
    t_idx_BT1 = tf.broadcast_to(
        tf.expand_dims(tf.range(audio_length), axis=0), [batch_size, audio_length]
    )
    ...
    b_idx_BTC = tf.broadcast_to(
        tf.reshape(tf.range(batch_size), [batch_size, 1, 1]),
        [batch_size, audio_length, C],
    )
    c_idx_BTC = tf.broadcast_to(
        tf.reshape(tf.range(C), [1, 1, C]), [batch_size, audio_length, C]
    )
...
def create_dataset(
    ...
    ds = ds.map(
        lambda text, audio: process_sample(text, audio, config),
    ...
    delay_precomp = build_delay_indices(
        config.batch_size, config.audio_length, config.delay_pattern
    )
