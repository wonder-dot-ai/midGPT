...
class Nari(eqx.Module):
    ...
    def __call__(self, src_T, tgt_T, inference=False, key=None):
        enc_key, dec_key, cfg_key = (None, None, None)
        if key is not None:
            enc_key, dec_key, cfg_key = jrandom.split(key, 3)

        encoder_out = self.encoder(src_T, inference=inference, key=enc_key)

        if not inference and cfg_key is not None:
            # During training, randomly drop encoder output with probability cfg_prob
            drop_mask = jrandom.bernoulli(cfg_key, p=self.cfg_prob)
            zeros = jnp.zeros_like(encoder_out)
            encoder_out = jnp.where(drop_mask, zeros, encoder_out)
